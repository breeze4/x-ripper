...
- [x] 5. Popup live progress: harvest-round progress messages from `src/content.js`, "Captured N posts…" in `src/popup/popup.js`; user ripped the mega-thread in Chrome and confirmed in chat ("Saved 82 text blocks and 1 image. Expanded 2 Show more links. Source: graphql. Embedded 1."); rip file verified — 82 entries, no leaks.
...
- [x] 9. Final proof in conversation: logged-out smoke output (all 6 PASS), logged-in smoke output with `graphql` on all four thread scenarios, `gh run list` green for the pushed head, `git log --oneline` showing the work on origin/main, and the user's popup confirmation. All gates met 2026-08-10.
